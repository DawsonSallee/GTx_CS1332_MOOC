...
public class BST<T extends Comparable<? super T>> {
    ...
    private BSTNode removeHelper(BSTNode<T> current, T data, BSTNode<T> dummyNode) {
        if(current == null) {
            return null;
        }
        ...
        if (compareResult > 0) {
            current.setLeft(removeHelper(current.getLeft(), data, dummyNode));
        } else if (compareResult < 0) {
            current.setRight(removeHelper(current.getRight(), data, dummyNode));
        } else {
    ...
    private BSTNode removeSuccessor(BSTNode current, BSTNode successorDummy) {
        if(current.getLeft() == null) {
            successorDummy.setData(current.getData());
            return current.getRight();
        } else {
            current.setLeft(removeSuccessor(current.getLeft(), successorDummy));
            return current;
        }
    }
